...
const PRODUCTION_HEALTH_DEPLOY_MARKER = "production_deploy_commit_checked_by_health=true";
    ...
    >
      ...
      <div className="d1-main-grid">
        <div className="d1-first-viewport-grid" data-d1-first-viewport-stage="E141-A">
          ...
          <div id="d1-technical-tab" className="d1-toolbar" aria-label="Вкладки D1">
            {tabIds.map((tabId) => (
          ...
          <span hidden>E141-A; chart_viewer_chart_and_graha_table_same_viewport=true; chart_viewer_inspector_same_viewport=true; chart_viewer_first_viewport_scroll_debt_reduced=true; backend_calculation_changed=false; {PRODUCTION_HEALTH_DEPLOY_MARKER}</span>
        </div>
...
function D1LaunchVisibilityStrip({ model }: { model: D1WorkbenchModel }) {
  ...
  return (
    <div className="d1-launch-visibility-strip" data-d1-launch-visibility-stage="E140-A" aria-label="Technical launch visibility">
      ...
      <span><strong>Production</strong>health deploy_commit checked by read-only smoke</span>
      <span hidden>E140-A; chart_viewer_launch_visibility_strip=true; chart_viewer_visible_technical_scope_summary=true; chart_viewer_d_scope_coverage_summary=true; chart_viewer_first_check_before_analysis_review=true; backend_calculation_changed=false; {PRODUCTION_HEALTH_DEPLOY_MARKER}</span>
    </div>
  );
}
...
function D1ScopeCoverageMatrix({
  ...
  return (
    <section className="d1-scope-matrix" data-d1-scope-matrix-stage="E143-A" aria-label="D1-D60 technical scope matrix">
      ...
      <span hidden>E143-A; chart_viewer_all_d_scopes_matrix_visible=true; chart_viewer_scope_matrix_quick_switch=true; chart_viewer_scope_matrix_availability_visible=true; chart_viewer_scope_matrix_grouped_by_use=true; backend_calculation_changed=false; {PRODUCTION_HEALTH_DEPLOY_MARKER}</span>
    </section>
  );
}
...
function D1ActiveScopeReviewStrip({
  ...
  return (
    <section className="d1-active-scope-review" data-d1-active-scope-review-stage="E144-A" aria-label="Active D-scope technical review">
      ...
      <span hidden>E144-A; chart_viewer_active_scope_method_visible=true; chart_viewer_active_scope_gate_visible=true; chart_viewer_active_scope_placement_count_visible=true; backend_calculation_changed=false; {PRODUCTION_HEALTH_DEPLOY_MARKER}</span>
    </section>
  );
}
...
function D1TechnicalPayloadIndex({ model, onOpenTechnical }: { model: D1WorkbenchModel; onOpenTechnical: () => void }) {
  ...
  const technicalPayloadIndexItems = [
    ...
    { key: "panchanga", label: "Panchanga", count: model.technical.panchanga.length },
    { key: "solar-day", label: "Solar day", count: model.technical.solarDay.length },
    { key: "dashas", label: "Dashas", count: model.technical.dashas.length },
    { key: "vargas", label: "Vargas", count: availableVargas, total: model.technical.vargas.length },
    { key: "house-cusps", label: "House cusps", count: model.technical.houseCusps.length },
    { key: "classical", label: "Classical", count: model.technical.classical.length },
  ];
  return (
    <section className="d1-technical-payload-index" data-d1-technical-payload-index-stage="E145-A" aria-label="Technical payload index">
      ...
      <div className="d1-technical-payload-index-grid">
        {technicalPayloadIndexItems.map((item) => (
          <span key={item.key} data-technical-payload-index-item={item.key}>
            <strong>{item.label}</strong>
            {item.total === undefined ? item.count : `${item.count}/${item.total}`}
          </span>
        ))}
      </div>
...
function D1MobileWorkflowNav({ activeTab, onGrahaJump }: { activeTab: D1DataTab; onGrahaJump: () => void }) {
  return (
    <nav className="d1-mobile-workflow-nav" data-d1-mobile-workflow-stage="E139-A" aria-label="Mobile chart workflow">
      ...
      <span hidden>E139-A; chart_viewer_mobile_workflow_nav=true; chart_viewer_mobile_chart_table_inspector_anchors=true; chart_viewer_mobile_graha_rows_compact=true; chart_viewer_mobile_no_wide_table_primary=true; backend_calculation_changed=false; {PRODUCTION_HEALTH_DEPLOY_MARKER}</span>
    </nav>
  );
}
...
function D1TechnicalContextStrip({
  ...
  return (
    <div className="chart-context-strip d1-technical-context-strip" data-d1-technical-context-stage="E137-A" aria-label="Technical chart context">
      ...
      <span hidden>E137-A; d1_technical_context_strip_present=true; chart_viewer_scope_coverage_visible=true; chart_viewer_calculation_status_visible=true; chart_viewer_accuracy_gate_visible=true; backend_calculation_changed=false; {PRODUCTION_HEALTH_DEPLOY_MARKER}</span>
      <span hidden>E138-A; chart_viewer_default_tab=grahas; chart_viewer_all_planets_visible_first_view=true; backend_calculation_changed=false; {PRODUCTION_HEALTH_DEPLOY_MARKER}</span>
    </div>
  );
}
