...
for (const marker of ["witness_dasha_parity", "level_summary", "target_met", "next_actions"]) {
...
for (const marker of [
  ...
  assert(panel.includes(marker), `AccuracyReportPanel missing dasha parity UI marker: ${marker}`);
}
...
for (const changed of changedFiles) {
  assert(
    ...
    `Formula/calculation file changed during dasha parity UI stage: ${changed}`,
  );
}

console.log("Accuracy dasha parity UI check passed.");
