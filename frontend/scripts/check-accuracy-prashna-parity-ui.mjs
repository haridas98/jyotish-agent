...
for (const marker of ["witness_prashna_parity", "layer_summary", "field_summary", "target_met", "tolerance_profile", "next_actions"]) {
...
for (const marker of [
  ...
  "Prashna parity",
  "Horary baseline rows",
  "prashnaParitySummary",
  "prashnaParityActions",
  "checked_fields",
  "failed_fields",
  "missing_fields",
  "skipped_fields",
]) {
  assert(panel.includes(marker), `AccuracyReportPanel missing Prashna parity UI marker: ${marker}`);
}

const prashnaSlice = sliceBetween(panel, "const prashnaParity", "const plFailedCount");
for (const marker of ["Р Вµ", "Р Р…", "Р С•", "Р Т‘", "РЎРѓ", "РЎвЂ ", "Р’В·", "пїЅ"]) {
  assert(!prashnaSlice.includes(marker), `Prashna parity slice contains mojibake marker: ${marker}`);
}
...
for (const forbidden of [
  ...
  assert(!prashnaSlice.includes(forbidden), `Prashna parity slice must not expose ${forbidden}`);
}
...
for (const changed of changedFiles) {
  assert(
    ...
    `Formula/workflow/report implementation file changed during Prashna parity UI stage: ${changed}`,
  );
}

console.log("Accuracy Prashna parity UI check passed.");
