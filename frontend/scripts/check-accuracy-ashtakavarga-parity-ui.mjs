...
for (const marker of ["witness_ashtakavarga_parity", "layer_summary", "body_summary", "target_met", "next_actions"]) {
...
for (const marker of [
  ...
  "ashtakavargaParityActions",
  "layer_summary",
  "body_summary",
  "checked_layers",
  "not_comparable_count",
]) {
  assert(panel.includes(marker), `AccuracyReportPanel missing Ashtakavarga parity UI marker: ${marker}`);
}

const ashtakavargaSlice = sliceBetween(panel, "const ashtakavargaParity", "const plFailedCount");
for (const marker of ["Рµ", "РЅ", "Рѕ", "Рґ", "СЃ", "С†", "В·", "�"]) {
  assert(!ashtakavargaSlice.includes(marker), `Ashtakavarga parity slice contains mojibake marker: ${marker}`);
}
...
for (const changed of changedFiles) {
  assert(
    ...
    `Formula/calculation file changed during Ashtakavarga parity UI stage: ${changed}`,
  );
}

console.log("Accuracy Ashtakavarga parity UI check passed.");
