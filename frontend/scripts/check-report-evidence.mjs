...
const files = {
  ...
  validation: readFileSync("src/astrology/reports/reportEvidenceValidation.ts", "utf8"),
  reportsIndex: readFileSync("src/astrology/reports/index.ts", "utf8"),
  debugPage: readFileSync("src/app/report-evidence-debug/page.tsx", "utf8"),
...
for (const marker of [
  "validateReportEvidencePack",
  "schemaVersion",
  "Duplicate evidence item id",
  "deterministically sorted",
  "sourceRefs",
  "unavailableItemIds",
]) {
  assert(files.validation.includes(marker), `Report evidence validation missing ${marker}`);
}
...
for (const forbidden of [
  ...
  assert(!files.validation.includes(forbidden), `Report evidence validation must not contain ${forbidden}`);
}
...
assert(files.reportsIndex.includes("./reportEvidenceValidation"), "Reports index must export evidence validation");
assert(files.debugPage.includes("debugRoutesEnabled"), "Evidence debug route must be guarded");
assert(files.debugPage.includes("redirect(\"/charts\")"), "Evidence debug route must redirect when disabled");
