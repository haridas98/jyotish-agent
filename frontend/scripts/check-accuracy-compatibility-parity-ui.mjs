...
for (const marker of ["witness_compatibility_parity", "layer_summary", "kuta_summary", "target_met", "tolerance_profile", "next_actions"]) {
...
for (const marker of [
  ...
  assert(panel.includes(marker), `AccuracyReportPanel missing Compatibility parity UI marker: ${marker}`);
}

const compatibilitySlice = sliceBetween(panel, "const compatibilityParity", "const plFailedCount");
for (const marker of ["Рµ", "РЅ", "Рѕ", "Рґ", "СЃ", "С†", "В·", "�"]) {
  assert(!compatibilitySlice.includes(marker), `Compatibility parity slice contains mojibake marker: ${marker}`);
}
...
for (const forbidden of [
  ...
  assert(!compatibilitySlice.includes(forbidden), `Compatibility parity slice must not expose ${forbidden}`);
}
...
for (const changed of changedFiles) {
  assert(
    ...
    `Formula/workflow/report implementation file changed during Compatibility parity UI stage: ${changed}`,
  );
}

console.log("Accuracy Compatibility parity UI check passed.");
