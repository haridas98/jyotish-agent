...
for (const marker of [
  ...
  "strengthsParityActions",
  "profile_sensitive_layers",
  "checked_fields",
  "failed_fields",
  "missing_fields",
]) {
  assert(panel.includes(marker), `AccuracyReportPanel missing Strengths parity UI marker: ${marker}`);
}

const strengthsSlice = sliceBetween(panel, "const strengthsParity", "const plFailedCount");
for (const marker of ["Р Вµ", "Р Р…", "Р С•", "Р Т‘", "РЎРѓ", "РЎвЂ ", "Р’В·", "пїЅ"]) {
  assert(!strengthsSlice.includes(marker), `Strengths parity slice contains mojibake marker: ${marker}`);
}
...
for (const changed of changedFiles) {
  assert(
    ...
    `Formula/calculation file changed during Strengths parity UI stage: ${changed}`,
  );
}

console.log("Accuracy Strengths parity UI check passed.");
