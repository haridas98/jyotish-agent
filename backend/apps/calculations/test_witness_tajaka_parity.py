...
def _tajaka_payload(*, mismatch: bool = False, optional_unknown: bool = False) -> dict:
    payload = {
        "status": "baseline_only" if not mismatch else "needs_review",
        "target_year": 2026,
        "tithi_pravesha": {"target_year": 2026},
        "tajaka": {
            "status": "baseline_only",
            "annual_lagna": {"rashi": "Karka" if not mismatch else "Simha", "degree": 12.5},
            "annual_sun": {"rashi": "Dhanu", "degree": 17.2},
            "annual_moon": {"rashi": "Makara", "degree": 11.2},
            "muntha": {"rashi": "Kanya" if not mismatch else "Tula", "house_from_annual_lagna": 3},
            "panchanga": {
            ...
            if not mismatch
            else ["tajaka_yogas", "sahams"],
        },
    }
    if optional_unknown:
        payload["experimental_tajaka_note"] = "operator note"
    return payload


def _manual_tajaka_payload() -> dict:
    return {
        "varshaphala": {
            "status": "baseline_only",
            "target_year": 2026,
            "annual_lagna": {"sign": "Karka", "degree": 12.5},
            "annual_sun": {"sign": "Dhanu", "degree": 17.2},
            "annual_moon": {"sign": "Makara", "degree": 11.2},
            "muntha": {"sign": "Kanya", "house": 3},
            "annual_panchanga": {
                "tithi": "Dvitiya",
                "nakshatra": "Shravana",
                "yoga": "Siddha",
...
def _write_jhora_case(
    ...
    *,
    ...
    fixture = {
        ...
        "expected": expected if expected is not None else {"tajaka": _tajaka_payload()},
    }
    ...
    if chart_payload is not None:
        ...
        (case_dir / "jyotish-agent-chart.json").write_text(
            json.dumps({"tajaka_report": _tajaka_payload()}),
            encoding="utf-8",
        )
    return case_dir
...
def _write_pl_case(root, case_id: str, *, review_status: str = "reviewed", manual_values=None, chart_payload=None):
    ...
    fixture = {
        ...
        "manual_witness_values": manual_values if manual_values is not None else _manual_tajaka_payload(),
    }
    (case_dir / "fixture.json").write_text(json.dumps(fixture), encoding="utf-8")
    (case_dir / "jyotish-agent-chart.json").write_text(
        json.dumps(chart_payload if chart_payload is not None else {"tajaka_report": _tajaka_payload()}),
        encoding="utf-8",
    )
    return case_dir


def test_tajaka_parity_passes_status_year_lagna_bodies_muntha_panchanga_and_open_items(tmp_path):
    from apps.calculations.witness_tajaka_parity import build_witness_tajaka_parity_report

    _write_jhora_case(tmp_path / "jhora", "sterlitamak-1998-04-30-1345")

    report = build_witness_tajaka_parity_report(witness_dir=tmp_path / "jhora", pl_root=tmp_path / "pl7")
    row = next(item for item in report["cases"] if item["case_id"] == "sterlitamak-1998-04-30-1345")

    assert report["schema_version"] == "jyotish-tajaka-parity-report-v1"
    ...
    assert report["field_summary"]["muntha.rashi"]["passed"] == 1
    assert report["field_summary"]["open_items"]["passed"] == 1


def test_tajaka_parity_fails_wrong_lagna_muntha_panchanga_and_open_item(tmp_path):
    from apps.calculations.witness_tajaka_parity import build_witness_tajaka_parity_report

    _write_jhora_case(
        tmp_path / "jhora",
        "sterlitamak-1998-04-30-1345",
        expected={"tajaka": _tajaka_payload(mismatch=True)},
    )

    row = next(
        item
        for item in build_witness_tajaka_parity_report(witness_dir=tmp_path / "jhora", pl_root=tmp_path / "pl7")[
            "cases"
        ]
    ...
    assert "annual_lagna.rashi" in row["failed_fields"]
    assert "muntha.rashi" in row["failed_fields"]
    assert "annual_panchanga.tithi" in row["failed_fields"]
    assert "open_items" in row["failed_fields"]


def test_tajaka_parity_passes_pl_manual_normalized_varshaphala(tmp_path):
    from apps.calculations.witness_tajaka_parity import build_witness_tajaka_parity_report
    ...
    assert report["layer_summary"]["muntha"]["passed"] == 1


def test_tajaka_parity_missing_witness_or_actual_is_not_formula_failure(tmp_path):
    from apps.calculations.witness_tajaka_parity import build_witness_tajaka_parity_report
    ...
    assert "jhora.tajaka" in rows["sterlitamak-1998-04-30-1345"]["missing_fields"]
    assert rows["mayapur-2001-02-03-0910"]["comparison_status"] == "not_comparable"
    assert "calculated.tajaka" in rows["mayapur-2001-02-03-0910"]["missing_fields"]
    assert report["summary"]["failed_count"] == 0


def test_tajaka_parity_skips_unknown_optional_fields_without_demoting_case(tmp_path):
    from apps.calculations.witness_tajaka_parity import build_witness_tajaka_parity_report

    _write_jhora_case(
        tmp_path / "jhora",
        "sterlitamak-1998-04-30-1345",
        expected={"varshaphala": _tajaka_payload(optional_unknown=True)},
    )
    ...
    assert row["comparison_status"] == "passed"
    assert "tajaka.experimental_tajaka_note" in row["skipped_fields"]
    assert report["field_summary"]["experimental_tajaka_note"]["skipped"] == 1


def test_tajaka_parity_computes_actual_from_fixture_annual_return_inputs(tmp_path, monkeypatch):
    from apps.calculations import workflows
    from apps.calculations.witness_tajaka_parity import build_witness_tajaka_parity_report

    def fake_build_tajaka_report(data):
        assert {"birth_date", "birth_time", "place_name", "target_year"} <= set(data)
        return _tajaka_payload()

    monkeypatch.setattr(workflows, "build_tajaka_report", fake_build_tajaka_report)
    _write_jhora_case(
        tmp_path / "jhora",
        "sterlitamak-1998-04-30-1345",
...
def test_tajaka_parity_unreviewed_case_is_not_counted_as_passed(tmp_path):
    from apps.calculations.witness_tajaka_parity import build_witness_tajaka_parity_report
...
def test_tajaka_parity_command_writes_json_markdown_and_safe_json_stdout(tmp_path):
    _write_jhora_case(tmp_path / "witness", "sterlitamak-1998-04-30-1345")
    output = tmp_path / "tajaka-parity.json"
    markdown = tmp_path / "tajaka-parity.md"
    stdout = StringIO()
    ...
    assert payload["schema_version"] == "jyotish-tajaka-parity-report-v1"
    ...
    assert "Tajaka Parity Report" in markdown_text
...
def test_tajaka_parity_command_default_output_path_matches_contract():
    from apps.calculations.management.commands.build_witness_tajaka_parity_report import Command

    parser = Command().create_parser("manage.py", "build_witness_tajaka_parity_report")
...
def test_tajaka_parity_stage_does_not_change_formula_workflow_or_existing_report_files():
