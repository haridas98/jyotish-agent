...
] + RESULT_FAMILY_LABELS + AUDIT_FAMILY_LABELS + REMEDIATION_FAMILY_LABELS + OPERATOR_PACKET_FAMILY_LABELS


class ExternalReceiptManifestDecisionAuditOperatorHandoffSafeValidationResultAuditRemediationQueueOperatorPacketTests(
    SimpleTestCase
):
    def setUp(self):
        self.tmp_dir = Path(__file__).resolve().parents[3] / ".tmp" / "test-p99"
        self.tmp_dir.mkdir(parents=True, exist_ok=True)

    def test_operator_packets_preserve_p97_order_and_block_delivery(self):
        remediation_path = self._write_payload("safe-validation-result-audit-remediation-queue.json", _p97_payload())
        ...
        report = build_witness_core_evidence_external_receipt_manifest_decision_audit_operator_handoff_safe_validation_result_audit_remediation_queue_operator_packet_report(
            external_receipt_manifest_decision_audit_operator_handoff_safe_validation_result_audit_remediation_queue_report_path=remediation_path,
        )
        ...
        self.assertEqual(report["stage"], "P99-A")
        self.assertEqual(
            report["status"],
            (
        ...
        self.assertEqual(
            [row["case_id"] for row in report["safe_validation_result_audit_remediation_queue_operator_packet_case_rows"]],
            CASE_IDS,
        )
        self.assertEqual(
            [
                row["case_id"]
                for row in report["safe_validation_result_audit_remediation_queue_operator_packet_attachment_rows"][::2]
            ],
            CASE_IDS,
        )
        self.assertEqual(
            [
                row["evidence_family"]
                for row in report["safe_validation_result_audit_remediation_queue_operator_packet_attachment_rows"][:4]
            ],
        ...
        self.assertEqual(
            [
                row["safe_validation_result_audit_remediation_queue_operator_packet_family_label"]
                for row in report["safe_validation_result_audit_remediation_queue_operator_packet_rows"][:4]
            ],
            OPERATOR_PACKET_FAMILY_LABELS,
        )
        self.assertTrue(
            all(
                row["safe_validation_result_audit_remediation_queue_operator_packet_delivery_status"] == "not_delivered"
                for row in report["safe_validation_result_audit_remediation_queue_operator_packet_rows"]
            )
        )
        self.assertTrue(
            all(
                row["safe_validation_result_audit_remediation_queue_operator_packet_acknowledgement_status"]
                == "not_acknowledged"
                for row in report["safe_validation_result_audit_remediation_queue_operator_packet_rows"]
            )
        )
        self.assertTrue(
            all(
                row["safe_validation_result_audit_remediation_queue_operator_packet_execution_status"] == "not_executed"
                for row in report["safe_validation_result_audit_remediation_queue_operator_packet_rows"]
            )
        )
        self.assertTrue(
            all(row["ready_to_attach"] is False for row in report["safe_validation_result_audit_remediation_queue_operator_packet_rows"])
        )
        self.assertTrue(
            all(row["ready_to_mark"] is False for row in report["safe_validation_result_audit_remediation_queue_operator_packet_rows"])
        )
        self.assertTrue(_contains_exact_marker_labels(report))
        self.assertTrue(_serialized_safe(report))

    def test_command_writes_json_without_mutating_p97_or_running_external_actions(self):
        remediation_path = self._write_payload("safe-validation-result-audit-remediation-queue.json", _p97_payload())
        output_path = self.tmp_dir / "safe-validation-result-audit-remediation-queue-operator-packet.json"
        before = remediation_path.read_text(encoding="utf-8")

        with (
            mock.patch("subprocess.Popen", side_effect=AssertionError("P99 must not run external tools")),
            mock.patch("subprocess.run", side_effect=AssertionError("P99 must not run commands")),
            mock.patch("os.system", side_effect=AssertionError("P99 must not run shell commands")),
        ):
            call_command(
                "build_witness_core_evidence_external_receipt_manifest_decision_audit_operator_handoff_safe_validation_result_audit_remediation_queue_operator_packet_report",
        ...
        self.assertEqual(written["stage"], "P99-A")
        ...
        self.assertEqual(remediation_path.read_text(encoding="utf-8"), before)

    def test_committed_p99_artifact_is_safe_and_current(self):
        ...
        self.assertEqual(
            [row["case_id"] for row in remediation["safe_validation_result_audit_remediation_case_rows"]],
            CASE_IDS,
        )
        self.assertEqual(len(report["safe_validation_result_audit_remediation_queue_operator_packet_rows"]), 40)
        self.assertTrue(_contains_exact_marker_labels(report))
    ...
    def test_stage_does_not_change_forbidden_files(self):
        ...
        allowed = {
            ...
            (
                ...
                "witness_core_evidence_external_receipt_manifest_decision_audit_operator_handoff_safe_validation_result_audit_remediation_queue_operator_packet.py"
            ),
            (
                "backend/apps/calculations/management/commands/"
                "build_witness_core_evidence_external_receipt_manifest_decision_audit_operator_handoff_safe_validation_result_audit_remediation_queue_operator_packet_report.py"
            ),
            (
                "backend/apps/calculations/"
                "test_witness_core_evidence_external_receipt_manifest_decision_audit_operator_handoff_safe_validation_result_audit_remediation_queue_operator_packet.py"
            ),
        }
        for path in changed:
...
def _expected_summary():
    return {
        "source_safe_validation_result_audit_remediation_case_rows": 5,
        "source_safe_validation_result_audit_remediation_attachment_rows": 10,
