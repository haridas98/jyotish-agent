...
from apps.calculations.witness_core_evidence_external_receipt_manifest_decision_audit_operator_handoff_safe_validation_result_audit_remediation_queue_operator_packet_dispatch_gate import (
    build_witness_core_evidence_external_receipt_manifest_decision_audit_operator_handoff_safe_validation_result_audit_remediation_queue_operator_packet_dispatch_gate_report,
)
...
] + RESULT_FAMILY_LABELS + AUDIT_FAMILY_LABELS + REMEDIATION_FAMILY_LABELS + OPERATOR_PACKET_FAMILY_LABELS + DISPATCH_GATE_FAMILY_LABELS


class ExternalReceiptManifestDecisionAuditOperatorHandoffSafeValidationResultAuditRemediationQueueOperatorPacketDispatchGateTests(
    SimpleTestCase
):
    def setUp(self):
        self.tmp_dir = Path(__file__).resolve().parents[3] / ".tmp" / "test-p101"
        self.tmp_dir.mkdir(parents=True, exist_ok=True)

    def test_dispatch_gate_preserves_p99_order_and_blocks_dispatch(self):
        operator_packet_path = _p99_artifact_path()

        report = build_witness_core_evidence_external_receipt_manifest_decision_audit_operator_handoff_safe_validation_result_audit_remediation_queue_operator_packet_dispatch_gate_report(
            external_receipt_manifest_decision_audit_operator_handoff_safe_validation_result_audit_remediation_queue_operator_packet_report_path=operator_packet_path,
        )
        ...
        self.assertEqual(report["stage"], "P101-A")
        self.assertEqual(
            report["status"],
            (
        ...
        self.assertEqual(
            [
                row["case_id"]
                for row in report["safe_validation_result_audit_remediation_queue_operator_packet_dispatch_gate_case_rows"]
            ],
            CASE_IDS,
        )
        self.assertEqual(
            [
                row["case_id"]
                for row in report["safe_validation_result_audit_remediation_queue_operator_packet_dispatch_gate_attachment_rows"][::2]
            ],
            CASE_IDS,
        )
        self.assertEqual(
            [
                row["evidence_family"]
                for row in report["safe_validation_result_audit_remediation_queue_operator_packet_dispatch_gate_attachment_rows"][:4]
            ],
        ...
        self.assertEqual(
            [
                row["safe_validation_result_audit_remediation_queue_operator_packet_dispatch_gate_family_label"]
                for row in report["safe_validation_result_audit_remediation_queue_operator_packet_dispatch_gate_rows"][:4]
            ],
            DISPATCH_GATE_FAMILY_LABELS,
        )
        self.assertTrue(
            all(
                row["safe_validation_result_audit_remediation_queue_operator_packet_dispatch_status"] == "not_dispatched"
                for row in report["safe_validation_result_audit_remediation_queue_operator_packet_dispatch_gate_rows"]
            )
        )
        self.assertTrue(
            all(
                row["safe_validation_result_audit_remediation_queue_operator_packet_dispatch_gate_delivery_status"]
                == "not_delivered"
                for row in report["safe_validation_result_audit_remediation_queue_operator_packet_dispatch_gate_rows"]
            )
        )
        self.assertTrue(
            all(
                row["safe_validation_result_audit_remediation_queue_operator_packet_dispatch_gate_execution_status"]
                == "not_executed"
                for row in report["safe_validation_result_audit_remediation_queue_operator_packet_dispatch_gate_rows"]
            )
        )
        self.assertTrue(
            all(
                row["ready_to_attach"] is False
                for row in report["safe_validation_result_audit_remediation_queue_operator_packet_dispatch_gate_rows"]
            )
        )
        self.assertTrue(
            all(
                row["ready_to_mark"] is False
                for row in report["safe_validation_result_audit_remediation_queue_operator_packet_dispatch_gate_rows"]
            )
        )
    ...
    def test_command_writes_json_without_mutating_p99_or_running_external_actions(self):
        operator_packet_path = _p99_artifact_path()
        output_path = self.tmp_dir / "safe-validation-result-audit-remediation-queue-operator-packet-dispatch-gate.json"
        before = operator_packet_path.read_text(encoding="utf-8")

        with (
            mock.patch("subprocess.Popen", side_effect=AssertionError("P101 must not run external tools")),
            mock.patch("subprocess.run", side_effect=AssertionError("P101 must not run commands")),
            mock.patch("os.system", side_effect=AssertionError("P101 must not run shell commands")),
        ):
            call_command(
                "build_witness_core_evidence_external_receipt_manifest_decision_audit_operator_handoff_safe_validation_result_audit_remediation_queue_operator_packet_dispatch_gate_report",
                "--external-receipt-manifest-decision-audit-operator-handoff-safe-validation-result-audit-remediation-queue-operator-packet-report",
        ...
        self.assertEqual(written["stage"], "P101-A")
        ...
        self.assertEqual(operator_packet_path.read_text(encoding="utf-8"), before)

    def test_committed_p101_artifact_is_safe_and_current(self):
        report = _read_json(_p101_artifact_path())
        operator_packet = _read_json(_p99_artifact_path())
        ...
        self.assertEqual(
            [row["case_id"] for row in operator_packet["safe_validation_result_audit_remediation_queue_operator_packet_case_rows"]],
            CASE_IDS,
        )
        self.assertEqual(
            len(report["safe_validation_result_audit_remediation_queue_operator_packet_dispatch_gate_rows"]),
    ...
    def test_stage_does_not_change_forbidden_files(self):
        ...
        allowed = {
            ...
            (
                ...
                "witness_core_evidence_external_receipt_manifest_decision_audit_operator_handoff_safe_validation_result_audit_remediation_queue_operator_packet_dispatch_gate.py"
            ),
            (
                "backend/apps/calculations/management/commands/"
                "build_witness_core_evidence_external_receipt_manifest_decision_audit_operator_handoff_safe_validation_result_audit_remediation_queue_operator_packet_dispatch_gate_report.py"
            ),
            (
                "backend/apps/calculations/"
                "test_witness_core_evidence_external_receipt_manifest_decision_audit_operator_handoff_safe_validation_result_audit_remediation_queue_operator_packet_dispatch_gate.py"
            ),
        }
        for path in changed:
...
def _expected_summary():
    return {
        "source_safe_validation_result_audit_remediation_queue_operator_packet_case_rows": 5,
        "source_safe_validation_result_audit_remediation_queue_operator_packet_attachment_rows": 10,
        ...
        "safe_validation_result_audit_remediation_queue_operator_packet_family_count": 4,
        "safe_validation_result_audit_remediation_queue_operator_packet_ready_count": 0,
        "safe_validation_result_audit_remediation_queue_operator_packet_blocked_count": 40,
