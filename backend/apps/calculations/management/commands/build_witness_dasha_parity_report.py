...
from apps.calculations.witness_dasha_parity import (
    build_witness_dasha_parity_report,
    render_dasha_parity_markdown,
)


class Command(BaseCommand):
    help = "Build a diagnostic Vimshottari dasha parity report from reviewed witness packets."
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--output",
            default=str(settings.ROOT_DIR / ".tmp" / "witness-review" / "dasha-parity-report.json"),
        )
        parser.add_argument(
            "--markdown-output",
            default=str(settings.ROOT_DIR / ".tmp" / "witness-review" / "dasha-parity-report.md"),
        )
        parser.add_argument("--target-reviewed-count", type=int, default=20)
        parser.add_argument("--sequence-limit", type=int, default=9)
        parser.add_argument("--date-tolerance-days", type=int, default=1)
        parser.add_argument("--json", action="store_true")

    def handle(self, *args, **options):
        report = build_witness_dasha_parity_report(
            jhora_root=options["jhora_root"],
            pl_root=options["pl_root"],
            target_reviewed_count=options["target_reviewed_count"],
            sequence_limit=options["sequence_limit"],
        ...
        if markdown_output:
            ...
            markdown_path.write_text(render_dasha_parity_markdown(report), encoding="utf-8")
        if options["json"]:
