...
from apps.calculations.witness_core_parity import (
    build_witness_core_parity_report,
    render_core_parity_markdown,
)


class Command(BaseCommand):
    help = "Build a diagnostic core graha/Lagna parity report from reviewed witness packets."
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--output",
            default=str(settings.ROOT_DIR / ".tmp" / "witness-review" / "core-parity-report.json"),
        )
    ...
    def handle(self, *args, **options):
        report = build_witness_core_parity_report(
        ...
        if markdown_output:
            ...
            markdown_path.write_text(render_core_parity_markdown(report), encoding="utf-8")
        if options["json"]:
