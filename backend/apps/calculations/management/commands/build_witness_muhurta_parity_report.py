...
from apps.calculations.witness_muhurta_parity import (
    build_witness_muhurta_parity_report,
    render_muhurta_parity_markdown,
)


class Command(BaseCommand):
    help = "Build a diagnostic muhurta parity report from reviewed witness packets."
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--output",
            default=str(settings.ROOT_DIR / ".tmp" / "witness-review" / "muhurta-parity-report.json"),
        )
        parser.add_argument(
            "--markdown-output",
            default=str(settings.ROOT_DIR / ".tmp" / "witness-review" / "muhurta-parity-report.md"),
        )
        parser.add_argument("--target-reviewed-count", type=int, default=20)
        parser.add_argument("--json", action="store_true")

    def handle(self, *args, **options):
        report = build_witness_muhurta_parity_report(
