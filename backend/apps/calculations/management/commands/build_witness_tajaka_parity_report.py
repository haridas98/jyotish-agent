...
from apps.calculations.witness_tajaka_parity import (
    build_witness_tajaka_parity_report,
    render_tajaka_parity_markdown,
)


class Command(BaseCommand):
    help = "Build a diagnostic Tajaka parity report from reviewed witness packets."
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--output",
            default=str(settings.ROOT_DIR / ".tmp" / "witness-review" / "tajaka-parity-report.json"),
        )
        parser.add_argument(
            "--markdown-output",
            default=str(settings.ROOT_DIR / ".tmp" / "witness-review" / "tajaka-parity-report.md"),
        )
        parser.add_argument("--target-reviewed-count", type=int, default=20)
        parser.add_argument("--json", action="store_true")

    def handle(self, *args, **options):
        report = build_witness_tajaka_parity_report(
        ...
        if markdown_output:
            ...
            markdown_path.write_text(render_tajaka_parity_markdown(report), encoding="utf-8")
        if options["json"]:
            self.stdout.write(
                json.dumps(
                    {
