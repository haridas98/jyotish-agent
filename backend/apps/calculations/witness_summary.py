...
def build_witness_summary(
    *,
    ...
    witness_strengths_parity_report_path: str | Path = "",
    witness_yoga_parity_report_path: str | Path = "",
    witness_special_points_parity_report_path: str | Path = "",
    ...
    witness_tajaka_parity_report_path: str | Path = "",
    witness_prashna_parity_report_path: str | Path = "",
    witness_jaimini_karaka_parity_report_path: str | Path = "",
    witness_jaimini_varga_parity_report_path: str | Path = "",
    ...
    witness_strengths_parity = _witness_strengths_parity(witness_strengths_parity_report_path)
    witness_yoga_parity = _witness_yoga_parity(witness_yoga_parity_report_path)
    witness_special_points_parity = _witness_special_points_parity(witness_special_points_parity_report_path)
    ...
    witness_tajaka_parity = _witness_tajaka_parity(witness_tajaka_parity_report_path)
    witness_prashna_parity = _witness_prashna_parity(witness_prashna_parity_report_path)
    witness_jaimini_karaka_parity = _witness_jaimini_karaka_parity(witness_jaimini_karaka_parity_report_path)
    witness_jaimini_varga_parity = _witness_jaimini_varga_parity(witness_jaimini_varga_parity_report_path)
    ...
    return {
        ...
        "witness_strengths_parity": witness_strengths_parity,
        "witness_yoga_parity": witness_yoga_parity,
        "witness_special_points_parity": witness_special_points_parity,
        ...
        "witness_tajaka_parity": witness_tajaka_parity,
        "witness_prashna_parity": witness_prashna_parity,
        "witness_jaimini_karaka_parity": witness_jaimini_karaka_parity,
        "witness_jaimini_varga_parity": witness_jaimini_varga_parity,
        "jhora": jhora,
...
def _witness_varga_parity(path: str | Path) -> dict[str, Any]:
    source_report = str(path or "")
    if not source_report or not Path(source_report).exists():
        return {
            "available": False,
    ...
    if not isinstance(report, dict) or not isinstance(report.get("summary"), dict):
        return {
            ...
            "schema_version": str(report.get("schema_version") if isinstance(report, dict) else ""),
            "generated_at": "",
            "error": "invalid varga parity report schema",
            "summary": _empty_core_parity_summary(),
            "varga_summary": {},
            "target_met": False,
            "next_actions": [],
    ...
    return {
        ...
        "generated_at": str(metadata.get("generated_at") or ""),
        "summary": summary,
        "varga_summary": _safe_varga_parity_summary(report.get("varga_summary")),
        "target_met": bool(summary.get("target_met")),
        "next_actions": _varga_parity_next_actions(report.get("cases")),
    }


def _safe_varga_parity_summary(value: Any) -> dict[str, dict[str, int]]:
...
def _varga_parity_next_actions(cases: Any, *, limit: int = 5) -> list[dict[str, Any]]:
    if not isinstance(cases, list):
        return []
    actions: list[dict[str, Any]] = []
    for row in cases:
...
def _witness_dasha_parity(path: str | Path) -> dict[str, Any]:
    source_report = str(path or "")
    if not source_report or not Path(source_report).exists():
        return {
            "available": False,
    ...
    if not isinstance(report, dict) or not isinstance(report.get("summary"), dict):
        return {
            ...
            "schema_version": str(report.get("schema_version") if isinstance(report, dict) else ""),
            "generated_at": "",
            "error": "invalid dasha parity report schema",
            "summary": _empty_core_parity_summary(),
            "level_summary": {},
            "target_met": False,
            "next_actions": [],
    ...
    return {
        ...
        "generated_at": str(metadata.get("generated_at") or ""),
        "summary": summary,
        "level_summary": _safe_dasha_level_summary(report.get("level_summary")),
        "target_met": bool(summary.get("target_met")),
        "next_actions": _dasha_parity_next_actions(report.get("cases")),
    }


def _safe_dasha_level_summary(value: Any) -> dict[str, dict[str, int]]:
    result: dict[str, dict[str, int]] = {}
    if not isinstance(value, dict):
        return result
...
def _dasha_parity_next_actions(cases: Any, *, limit: int = 5) -> list[dict[str, Any]]:
    if not isinstance(cases, list):
        return []
    actions: list[dict[str, Any]] = []
    for row in cases:
        ...
        actions.append(
            {
                "case_id": str(row.get("case_id") or ""),
                "status": str(row.get("comparison_status") or "unknown"),
                "checked_levels": _string_list(row.get("checked_levels")),
                "failed_levels": _string_list(row.get("failed_levels")),
                "missing_levels": _string_list(row.get("missing_levels")),
                "missing_fields": _string_list(row.get("missing_fields"))[:12],
                "failed_fields": _string_list(row.get("failed_fields"))[:12],
            }
        )
...
def _witness_panchanga_parity(path: str | Path) -> dict[str, Any]:
    source_report = str(path or "")
    if not source_report or not Path(source_report).exists():
        return {
            "available": False,
    ...
    if not isinstance(report, dict) or not isinstance(report.get("summary"), dict):
        return {
            ...
            "schema_version": str(report.get("schema_version") if isinstance(report, dict) else ""),
            "generated_at": "",
            "error": "invalid panchanga parity report schema",
            "summary": _empty_core_parity_summary(),
            "field_summary": {},
            "target_met": False,
            "next_actions": [],
    ...
    return {
        ...
        "generated_at": str(metadata.get("generated_at") or ""),
        "summary": summary,
        "field_summary": _safe_panchanga_field_summary(report.get("field_summary")),
        "target_met": bool(summary.get("target_met")),
        "next_actions": _panchanga_parity_next_actions(report.get("cases")),
    }


def _safe_panchanga_field_summary(value: Any) -> dict[str, dict[str, int]]:
    result: dict[str, dict[str, int]] = {}
    if not isinstance(value, dict):
        return result
...
def _panchanga_parity_next_actions(cases: Any, *, limit: int = 5) -> list[dict[str, Any]]:
    if not isinstance(cases, list):
        return []
    actions: list[dict[str, Any]] = []
    for row in cases:
...
def _witness_strengths_parity(path: str | Path) -> dict[str, Any]:
    source_report = str(path or "")
    if not source_report or not Path(source_report).exists():
        return {
            "available": False,
    ...
    if not isinstance(report, dict) or not isinstance(report.get("summary"), dict):
        return {
            ...
            "schema_version": str(report.get("schema_version") if isinstance(report, dict) else ""),
            "generated_at": "",
            "error": "invalid strengths parity report schema",
            "summary": _empty_core_parity_summary(),
            "layer_summary": {},
            "body_summary": {},
            "target_met": False,
    ...
    return {
        ...
        "body_summary": _safe_parity_count_summary(report.get("body_summary")),
        "target_met": bool(summary.get("target_met")),
        "profile_sensitive_layers": _string_list(metadata.get("profile_sensitive_layers")),
        "next_actions": _strengths_parity_next_actions(report.get("cases")),
    }


def _strengths_parity_next_actions(cases: Any, *, limit: int = 5) -> list[dict[str, Any]]:
    if not isinstance(cases, list):
        return []
    actions: list[dict[str, Any]] = []
    for row in cases:
...
def _witness_yoga_parity(path: str | Path) -> dict[str, Any]:
    source_report = str(path or "")
    if not source_report or not Path(source_report).exists():
        return {
            "available": False,
    ...
    if not isinstance(report, dict) or not isinstance(report.get("summary"), dict):
        return {
            ...
            "schema_version": str(report.get("schema_version") if isinstance(report, dict) else ""),
            "generated_at": "",
            "error": "invalid yoga parity report schema",
            "summary": _empty_core_parity_summary(),
            "layer_summary": {},
            "yoga_summary": {},
            "target_met": False,
    ...
    return {
        ...
        "summary": summary,
        "layer_summary": _safe_parity_count_summary(report.get("layer_summary")),
        "yoga_summary": _safe_yoga_parity_summary(report.get("yoga_summary")),
        "target_met": bool(summary.get("target_met")),
        "next_actions": _yoga_parity_next_actions(report.get("cases")),
    }


def _safe_yoga_parity_summary(value: Any) -> dict[str, dict[str, int]]:
    result: dict[str, dict[str, int]] = {}
    if not isinstance(value, dict):
        return result
    for key, row in value.items():
...
def _yoga_parity_next_actions(cases: Any, *, limit: int = 5) -> list[dict[str, Any]]:
    if not isinstance(cases, list):
        return []
    actions: list[dict[str, Any]] = []
    for row in cases:
        ...
        actions.append(
            {
                ...
                "failed_layers": _string_list(row.get("failed_layers")),
                "missing_layers": _string_list(row.get("missing_layers")),
                "checked_yogas": _string_list(row.get("checked_yogas"))[:12],
                "matched_yogas": _string_list(row.get("matched_yogas"))[:12],
                "failed_yogas": _string_list(row.get("failed_yogas"))[:12],
                "missing_yogas": _string_list(row.get("missing_yogas"))[:12],
                "skipped_yogas": _string_list(row.get("skipped_yogas"))[:12],
                "missing_fields": _string_list(row.get("missing_fields"))[:12],
                "failed_fields": _string_list(row.get("failed_fields"))[:12],
            }
        )
...
def _witness_special_points_parity(path: str | Path) -> dict[str, Any]:
    source_report = str(path or "")
    if not source_report or not Path(source_report).exists():
        return {
            "available": False,
    ...
    if not isinstance(report, dict) or not isinstance(report.get("summary"), dict):
        return {
            ...
            "schema_version": str(report.get("schema_version") if isinstance(report, dict) else ""),
            "generated_at": "",
            "error": "invalid special points parity report schema",
            "summary": _empty_core_parity_summary(),
            "layer_summary": {},
            "point_summary": {},
            "target_met": False,
    ...
    return {
        ...
        "summary": summary,
        "layer_summary": _safe_parity_count_summary(report.get("layer_summary")),
        "point_summary": _safe_point_parity_summary(report.get("point_summary")),
        "target_met": bool(summary.get("target_met")),
        "next_actions": _special_points_parity_next_actions(report.get("cases")),
    }


def _safe_point_parity_summary(value: Any) -> dict[str, dict[str, int]]:
    result: dict[str, dict[str, int]] = {}
    if not isinstance(value, dict):
        return result
    for key, row in value.items():
...
def _special_points_parity_next_actions(cases: Any, *, limit: int = 5) -> list[dict[str, Any]]:
    if not isinstance(cases, list):
        return []
    actions: list[dict[str, Any]] = []
    for row in cases:
        ...
        actions.append(
            {
                ...
                "failed_layers": _string_list(row.get("failed_layers")),
                "missing_layers": _string_list(row.get("missing_layers")),
                "checked_points": _string_list(row.get("checked_points"))[:12],
                "matched_points": _string_list(row.get("matched_points"))[:12],
                "failed_points": _string_list(row.get("failed_points"))[:12],
                "missing_points": _string_list(row.get("missing_points"))[:12],
                "skipped_points": _string_list(row.get("skipped_points"))[:12],
                "missing_fields": _string_list(row.get("missing_fields"))[:12],
                "failed_fields": _string_list(row.get("failed_fields"))[:12],
            }
        )
...
def _witness_argala_parity(path: str | Path) -> dict[str, Any]:
    source_report = str(path or "")
    if not source_report or not Path(source_report).exists():
        return {
            "available": False,
    ...
    if not isinstance(report, dict) or not isinstance(report.get("summary"), dict):
        return {
            ...
            "schema_version": str(report.get("schema_version") if isinstance(report, dict) else ""),
            "generated_at": "",
            "error": "invalid argala parity report schema",
            "summary": _empty_core_parity_summary(),
            "layer_summary": {},
            "argala_summary": {},
            "target_met": False,
    ...
    return {
        ...
        "summary": summary,
        "layer_summary": _safe_parity_count_summary(report.get("layer_summary")),
        "argala_summary": _safe_argala_parity_summary(report.get("argala_summary")),
        "target_met": bool(summary.get("target_met")),
        "next_actions": _argala_parity_next_actions(report.get("cases")),
    }


def _safe_argala_parity_summary(value: Any) -> dict[str, dict[str, int]]:
    result: dict[str, dict[str, int]] = {}
    if not isinstance(value, dict):
        return result
    for key, row in value.items():
...
def _argala_parity_next_actions(cases: Any, *, limit: int = 5) -> list[dict[str, Any]]:
    if not isinstance(cases, list):
        return []
    actions: list[dict[str, Any]] = []
    for row in cases:
...
def _witness_avastha_parity(path: str | Path) -> dict[str, Any]:
    source_report = str(path or "")
    if not source_report or not Path(source_report).exists():
        return {
            "available": False,
    ...
    if not isinstance(report, dict) or not isinstance(report.get("summary"), dict):
        return {
            ...
            "schema_version": str(report.get("schema_version") if isinstance(report, dict) else ""),
            "generated_at": "",
            "error": "invalid avastha parity report schema",
            "summary": _empty_core_parity_summary(),
            "layer_summary": {},
            "avastha_summary": {},
            "target_met": False,
    ...
    return {
        ...
        "summary": summary,
        "layer_summary": _safe_parity_count_summary(report.get("layer_summary")),
        "avastha_summary": _safe_avastha_parity_summary(report.get("avastha_summary")),
        "target_met": bool(summary.get("target_met")),
        "next_actions": _avastha_parity_next_actions(report.get("cases")),
    }


def _safe_avastha_parity_summary(value: Any) -> dict[str, dict[str, int]]:
    result: dict[str, dict[str, int]] = {}
    if not isinstance(value, dict):
        return result
    for key, row in value.items():
...
def _avastha_parity_next_actions(cases: Any, *, limit: int = 5) -> list[dict[str, Any]]:
    if not isinstance(cases, list):
        return []
    actions: list[dict[str, Any]] = []
    for row in cases:
...
def _witness_drishti_parity(path: str | Path) -> dict[str, Any]:
    source_report = str(path or "")
    if not source_report or not Path(source_report).exists():
        return {
            "available": False,
    ...
    if not isinstance(report, dict) or not isinstance(report.get("summary"), dict):
        return {
            ...
            "schema_version": str(report.get("schema_version") if isinstance(report, dict) else ""),
            "generated_at": "",
            "error": "invalid drishti parity report schema",
            "summary": _empty_core_parity_summary(),
            "layer_summary": {},
            "drishti_summary": {},
            "target_met": False,
    ...
    return {
        ...
        "summary": summary,
        "layer_summary": _safe_parity_count_summary(report.get("layer_summary")),
        "drishti_summary": _safe_drishti_parity_summary(report.get("drishti_summary")),
        "target_met": bool(summary.get("target_met")),
        "next_actions": _drishti_parity_next_actions(report.get("cases")),
    }


def _safe_drishti_parity_summary(value: Any) -> dict[str, dict[str, int]]:
    result: dict[str, dict[str, int]] = {}
    if not isinstance(value, dict):
        return result
    for key, row in value.items():
...
def _drishti_parity_next_actions(cases: Any, *, limit: int = 5) -> list[dict[str, Any]]:
    if not isinstance(cases, list):
        return []
    actions: list[dict[str, Any]] = []
    for row in cases:
        ...
        actions.append(
            {
                ...
                "failed_layers": _string_list(row.get("failed_layers")),
                "missing_layers": _string_list(row.get("missing_layers")),
                "checked_aspects": _string_list(row.get("checked_aspects"))[:12],
                "matched_aspects": _string_list(row.get("matched_aspects"))[:12],
...
def _witness_compatibility_parity(path: str | Path) -> dict[str, Any]:
    source_report = str(path or "")
    if not source_report or not Path(source_report).exists():
        return {
            "available": False,
    ...
    if not isinstance(report, dict) or not isinstance(report.get("summary"), dict):
        return {
            ...
            "schema_version": str(report.get("schema_version") if isinstance(report, dict) else ""),
            "generated_at": "",
            "error": "invalid compatibility parity report schema",
            "summary": _empty_core_parity_summary(),
            "layer_summary": {},
            "kuta_summary": {},
            "target_met": False,
    ...
    return {
        ...
        "summary": summary,
        "layer_summary": _safe_parity_count_summary(report.get("layer_summary")),
        "kuta_summary": _safe_compatibility_kuta_summary(report.get("kuta_summary")),
        "target_met": bool(summary.get("target_met")),
        "tolerance_profile": _safe_float_map(metadata.get("tolerance_profile")),
        "next_actions": _compatibility_parity_next_actions(report.get("cases")),
    }


def _safe_compatibility_kuta_summary(value: Any) -> dict[str, dict[str, int]]:
    result: dict[str, dict[str, int]] = {}
    if not isinstance(value, dict):
        return result
    for key, row in value.items():
...
def _compatibility_parity_next_actions(cases: Any, *, limit: int = 5) -> list[dict[str, Any]]:
    if not isinstance(cases, list):
        return []
    actions: list[dict[str, Any]] = []
    for row in cases:
        ...
        actions.append(
            {
                ...
                "failed_layers": _string_list(row.get("failed_layers")),
                "missing_layers": _string_list(row.get("missing_layers")),
                "checked_kutas": _string_list(row.get("checked_kutas"))[:12],
                "matched_kutas": _string_list(row.get("matched_kutas"))[:12],
                "failed_kutas": _string_list(row.get("failed_kutas"))[:12],
                "missing_kutas": _string_list(row.get("missing_kutas"))[:12],
                "skipped_kutas": _string_list(row.get("skipped_kutas"))[:12],
                "checked_fields": _string_list(row.get("checked_fields"))[:12],
                "failed_fields": _string_list(row.get("failed_fields"))[:12],
                "missing_fields": _string_list(row.get("missing_fields"))[:12],
                "skipped_fields": _string_list(row.get("skipped_fields"))[:12],
...
def _witness_muhurta_parity(path: str | Path) -> dict[str, Any]:
    source_report = str(path or "")
    if not source_report or not Path(source_report).exists():
        return {
            "available": False,
    ...
    if not isinstance(report, dict) or not isinstance(report.get("summary"), dict):
        return {
            ...
            "schema_version": str(report.get("schema_version") if isinstance(report, dict) else ""),
            "generated_at": "",
            "error": "invalid muhurta parity report schema",
            "summary": _empty_core_parity_summary(),
            "layer_summary": {},
            "field_summary": {},
            "target_met": False,
    ...
    return {
        ...
        "target_met": bool(summary.get("target_met")),
        "tolerance_profile": _safe_float_map(metadata.get("tolerance_profile")),
        "next_actions": _muhurta_parity_next_actions(report.get("cases")),
    }


def _safe_muhurta_field_summary(value: Any) -> dict[str, dict[str, int]]:
    result: dict[str, dict[str, int]] = {}
    if not isinstance(value, dict):
        return result
    for key, row in value.items():
...
def _witness_tajaka_parity(path: str | Path) -> dict[str, Any]:
    source_report = str(path or "")
    if not source_report or not Path(source_report).exists():
        return {
            "available": False,
    ...
    if not isinstance(report, dict) or not isinstance(report.get("summary"), dict):
        return {
            ...
            "schema_version": str(report.get("schema_version") if isinstance(report, dict) else ""),
            "generated_at": "",
            "error": "invalid tajaka parity report schema",
            "summary": _empty_core_parity_summary(),
            "layer_summary": {},
            "field_summary": {},
            "target_met": False,
    ...
    return {
        ...
        "target_met": bool(summary.get("target_met")),
        "tolerance_profile": _safe_float_map(metadata.get("tolerance_profile")),
        "next_actions": _tajaka_parity_next_actions(report.get("cases")),
    }


def _tajaka_parity_next_actions(cases: Any, *, limit: int = 5) -> list[dict[str, Any]]:
    if not isinstance(cases, list):
        return []
    actions: list[dict[str, Any]] = []
    for row in cases:
...
def _witness_prashna_parity(path: str | Path) -> dict[str, Any]:
    source_report = str(path or "")
    if not source_report or not Path(source_report).exists():
        return {
            "available": False,
    ...
    if not isinstance(report, dict) or not isinstance(report.get("summary"), dict):
        return {
            ...
            "schema_version": str(report.get("schema_version") if isinstance(report, dict) else ""),
            "generated_at": "",
            "error": "invalid prashna parity report schema",
            "summary": _empty_core_parity_summary(),
            "layer_summary": {},
            "field_summary": {},
            "target_met": False,
    ...
    return {
        ...
        "target_met": bool(summary.get("target_met")),
        "tolerance_profile": _safe_float_map(metadata.get("tolerance_profile")),
        "next_actions": _prashna_parity_next_actions(report.get("cases")),
    }


def _prashna_parity_next_actions(cases: Any, *, limit: int = 5) -> list[dict[str, Any]]:
    if not isinstance(cases, list):
        return []
    actions: list[dict[str, Any]] = []
    for row in cases:
...
def _witness_jaimini_karaka_parity(path: str | Path) -> dict[str, Any]:
    source_report = str(path or "")
    if not source_report or not Path(source_report).exists():
        return {
            "available": False,
    ...
    if not isinstance(report, dict) or not isinstance(report.get("summary"), dict):
        return {
            ...
            "schema_version": str(report.get("schema_version") if isinstance(report, dict) else ""),
            "generated_at": "",
            "error": "invalid jaimini karaka parity report schema",
            "summary": _empty_core_parity_summary(),
            "layer_summary": {},
            "field_summary": {},
            "target_met": False,
    ...
    return {
        ...
        "target_met": bool(summary.get("target_met")),
        "tolerance_profile": _safe_float_map(metadata.get("tolerance_profile")),
        "next_actions": _jaimini_karaka_parity_next_actions(report.get("cases")),
    }


def _jaimini_karaka_parity_next_actions(cases: Any, *, limit: int = 5) -> list[dict[str, Any]]:
    if not isinstance(cases, list):
        return []
    actions: list[dict[str, Any]] = []
    for row in cases:
...
def _witness_jaimini_varga_parity(path: str | Path) -> dict[str, Any]:
    source_report = str(path or "")
    if not source_report or not Path(source_report).exists():
        return {
            "available": False,
    ...
    if not isinstance(report, dict) or not isinstance(report.get("summary"), dict):
        return {
            ...
            "schema_version": str(report.get("schema_version") if isinstance(report, dict) else ""),
            "generated_at": "",
            "error": "invalid jaimini varga parity report schema",
            "summary": _empty_core_parity_summary(),
            "varga_summary": {},
            "field_summary": {},
            "readiness_summary": {},
    ...
    return {
        ...
        "generated_at": str(metadata.get("generated_at") or ""),
        "summary": summary,
        "varga_summary": _safe_varga_gap_summary(report.get("varga_summary")),
        "field_summary": _safe_muhurta_field_summary(report.get("field_summary")),
        "readiness_summary": _safe_readiness_summary(report.get("readiness_summary")),
        "target_met": bool(summary.get("target_met")),
        "next_actions": _jaimini_varga_parity_next_actions(report.get("cases")),
    }


def _safe_varga_gap_summary(value: Any) -> dict[str, dict[str, int]]:
    result: dict[str, dict[str, int]] = {}
    if not isinstance(value, dict):
        return result
    for key, row in value.items():
...
def _safe_readiness_summary(value: Any) -> dict[str, dict[str, int]]:
    result: dict[str, dict[str, int]] = {}
    if not isinstance(value, dict):
        return result
    for key, row in value.items():
        if not isinstance(row, dict):
            continue
        result[str(key)] = {
            "compared": _safe_int(row.get("compared")),
            "actual_missing": _safe_int(row.get("actual_missing")),
        }
    return result


def _jaimini_varga_parity_next_actions(cases: Any, *, limit: int = 5) -> list[dict[str, Any]]:
    if not isinstance(cases, list):
        return []
    actions: list[dict[str, Any]] = []
    for row in cases:
        ...
        actions.append(
            {
                ...
                "failed_vargas": _string_list(row.get("failed_vargas")),
                "missing_vargas": _string_list(row.get("missing_vargas")),
                "skipped_vargas": _string_list(row.get("skipped_vargas")),
                "checked_fields": _string_list(row.get("checked_fields"))[:12],
                "failed_fields": _string_list(row.get("failed_fields"))[:12],
                "missing_fields": _string_list(row.get("missing_fields"))[:12],
                "skipped_fields": _string_list(row.get("skipped_fields"))[:12],
