...
BLOCKED_NOTE = (
    "Receipt manifest decision queue is blocked; no receipt manifest decision is recorded; "
    "rows are labels only and no accept, reject, defer, upload, attachment, or mark command is executed."
)
SLOT_BLOCKED_NOTE = (
    "Receipt manifest is not received; accept, reject, and defer decisions are not started; "
    "no evidence file, evidence hash, upload, attachment, or mark command was executed."
)


def build_witness_core_evidence_external_receipt_manifest_decision_queue_report(
    *,
    external_receipt_manifest_acceptance_gate_report_path: str | Path,
) -> dict[str, Any]:
    source = _read_json(Path(external_receipt_manifest_acceptance_gate_report_path))
    source_summary = source.get("summary") if isinstance(source.get("summary"), dict) else {}
    case_acceptance_rows = [
        row for row in source.get("case_receipt_manifest_acceptance_gate_rows", []) if isinstance(row, dict)
    ]
    slot_acceptance_rows = [
        row
        for row in source.get("attachment_receipt_manifest_acceptance_gate_rows", [])
        if isinstance(row, dict)
    ]
    slots_by_case = _slots_by_case(slot_acceptance_rows)

    case_rows = [
        _case_decision_row(
            row,
            case_receipt_manifest_decision_queue_index=index,
            source_slots=slots_by_case.get(
                _safe_int(row.get("case_receipt_manifest_acceptance_gate_index")) or index,
                [],
            ),
        )
        for index, row in enumerate(case_acceptance_rows, start=1)
    ]
    slot_rows = [
        _attachment_decision_row(row, attachment_receipt_manifest_decision_queue_index=index)
        for index, row in enumerate(slot_acceptance_rows, start=1)
    ]
    ...
    return {
        ...
        "decision_criteria_labels": DECISION_CRITERIA_LABELS,
        ...
        "summary": {
            "source_case_receipt_manifest_acceptance_gate_rows": _safe_int(
                source_summary.get("case_receipt_manifest_acceptance_gate_rows", len(case_acceptance_rows))
            ),
            "source_attachment_receipt_manifest_acceptance_gate_rows": _safe_int(
                source_summary.get(
                    "attachment_receipt_manifest_acceptance_gate_rows",
                    len(slot_acceptance_rows),
                )
            ),
            "case_receipt_manifest_decision_queue_rows": len(case_rows),
            "attachment_receipt_manifest_decision_queue_rows": len(slot_rows),
            "pending_external_evidence_receipt_manifest_decision_count": len(slot_rows),
            "pending_jhora_receipt_manifest_decision_count": sum(
                1 for row in slot_rows if row["evidence_family"] == "jhora_screenshot_or_packet"
            ),
            "pending_parashara_light_receipt_manifest_decision_count": sum(
                1
...
def _case_decision_row(
    row: dict[str, Any],
    *,
    case_receipt_manifest_decision_queue_index: int,
    source_slots: list[dict[str, Any]],
) -> dict[str, Any]:
    source_slot_indices = [
        _safe_int(slot.get("attachment_receipt_manifest_acceptance_gate_index")) for slot in source_slots
    ]
    families = {str(slot.get("evidence_family") or "") for slot in source_slots}
    return {
        "case_receipt_manifest_decision_queue_index": case_receipt_manifest_decision_queue_index,
        "case_id": str(row.get("case_id") or ""),
        "source_case_receipt_manifest_acceptance_gate_index": _safe_int(
            row.get("case_receipt_manifest_acceptance_gate_index")
        )
        or case_receipt_manifest_decision_queue_index,
        "source_attachment_receipt_manifest_acceptance_gate_indices": source_slot_indices,
        "decision_queue_status": STATUS,
        ...
        "required_external_evidence_receipt_manifest_decisions": len(REQUIRED_EVIDENCE_FAMILIES),
        "pending_external_evidence_receipt_manifest_decisions": len(REQUIRED_EVIDENCE_FAMILIES),
        "jhora_decision_required": "jhora_screenshot_or_packet" in families,
        "parashara_light_decision_required": "parashara_light_manual_values_or_packet" in families,
...
def _attachment_decision_row(
    row: dict[str, Any],
    *,
    attachment_receipt_manifest_decision_queue_index: int,
) -> dict[str, Any]:
    family = _safe_evidence_family(row.get("evidence_family"))
    return {
        "attachment_receipt_manifest_decision_queue_index": attachment_receipt_manifest_decision_queue_index,
        "case_receipt_manifest_decision_queue_index": _safe_int(
            row.get("case_receipt_manifest_acceptance_gate_index")
        ),
        "source_attachment_receipt_manifest_acceptance_gate_index": _safe_int(
            row.get("attachment_receipt_manifest_acceptance_gate_index")
        ),
        "case_id": str(row.get("case_id") or ""),
...
def _slots_by_case(rows: list[dict[str, Any]]) -> dict[int, list[dict[str, Any]]]:
    ...
    for row in rows:
        index = _safe_int(row.get("case_receipt_manifest_acceptance_gate_index"))
        if index:
            grouped[index].append(row)
    for case_rows in grouped.values():
        case_rows.sort(key=lambda row: _safe_int(row.get("attachment_receipt_manifest_acceptance_gate_index")))
    return dict(grouped)
