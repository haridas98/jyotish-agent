...
from apps.calculations.witness_core_evidence_external_receipt_manifest_decision_audit_work_order_readiness import (
...
class ExternalReceiptManifestDecisionAuditWorkOrderReadinessTests(SimpleTestCase):
    def test_readiness_packets_preserve_p85_order_and_block_all_external_actions(self):
        work_orders_path = self._write_payload("decision-audit-work-orders.json", _p85_payload())

        report = build_witness_core_evidence_external_receipt_manifest_decision_audit_work_order_readiness_report(
            external_receipt_manifest_decision_audit_work_orders_report_path=work_orders_path,
        )
        ...
        self.assertEqual(report["stage"], "P87-A")
        self.assertEqual(
            report["status"],
            "blocked_pending_external_evidence_receipt_manifest_decision_audit_work_order_readiness",
        )
        self.assertEqual(
            report["upstream_decision_audit_work_orders_schema_version"],
        ...
        self.assertEqual(
            report["upstream_decision_audit_work_orders_status"],
        ...
        self.assertEqual(
            [row["case_id"] for row in report["operator_handoff_case_packet_rows"]],
            CASE_IDS,
        )
        self.assertEqual(
            [row["case_id"] for row in report["operator_handoff_attachment_packet_rows"][::2]],
            CASE_IDS,
        )
        self.assertEqual(
            [row["evidence_family"] for row in report["operator_handoff_attachment_packet_rows"][:4]],
            FAMILY_ORDER + FAMILY_ORDER,
        )
        self.assertEqual(
            report["operator_handoff_case_packet_rows"][0]["operator_handoff_attachment_packet_indices"],
            [1, 2],
        )
        self.assertEqual(
            report["operator_handoff_case_packet_rows"][-1]["operator_handoff_attachment_packet_indices"],
            [9, 10],
        )
        self.assertTrue(
            all(
                row["decision_audit_work_order_readiness_status"]
                == "blocked_pending_external_evidence_receipt_manifest_decision_audit_work_order_readiness"
                for row in report["operator_handoff_case_packet_rows"]
            )
        )
        self.assertTrue(
            all(row["operator_handoff_status"] == "not_delivered" for row in report["operator_handoff_attachment_packet_rows"])
        )
        self.assertTrue(
            all(row["work_order_delivery_status"] == "not_delivered" for row in report["operator_handoff_attachment_packet_rows"])
        )
        self.assertTrue(
            all(row["human_decision_audit_status"] == "not_started" for row in report["operator_handoff_attachment_packet_rows"])
        )
        self.assertTrue(all(row["ready_to_attach"] is False for row in report["operator_handoff_attachment_packet_rows"]))
        self.assertTrue(all(row["ready_to_mark"] is False for row in report["operator_handoff_attachment_packet_rows"]))
        self.assertTrue(all(row["no_work_order_delivery_executed"] is True for row in report["operator_handoff_attachment_packet_rows"]))
        self.assertTrue(all(row["no_operator_handoff_delivered"] is True for row in report["operator_handoff_attachment_packet_rows"]))
        self.assertIn(
    ...
    def test_command_writes_json_without_mutating_p85_or_running_external_actions(self):
        work_orders_path = self._write_payload("decision-audit-work-orders.json", _p85_payload())
        output_path = self.tmp_dir / "decision-audit-work-order-readiness.json"
        before = work_orders_path.read_text(encoding="utf-8")

        with mock.patch("subprocess.Popen", side_effect=AssertionError("P87 must not run external tools")):
            call_command(
                "build_witness_core_evidence_external_receipt_manifest_decision_audit_work_order_readiness_report",
                "--external-receipt-manifest-decision-audit-work-orders-report",
        ...
        self.assertEqual(work_orders_path.read_text(encoding="utf-8"), before)

    def test_committed_p87_artifact_is_safe_and_current(self):
        ...
        self.assertEqual(report["summary"], _expected_summary())
        self.assertEqual([row["case_id"] for row in report["operator_handoff_case_packet_rows"]], CASE_IDS)
        self.assertEqual([row["case_id"] for row in work_orders["case_decision_audit_work_order_rows"]], CASE_IDS)
        self.assertEqual([row["case_id"] for row in report["operator_handoff_attachment_packet_rows"][::2]], CASE_IDS)
        self.assertTrue(
            all(row["operator_handoff_status"] == "not_delivered" for row in report["operator_handoff_attachment_packet_rows"])
        )
        self.assertTrue(
    ...
    def test_stage_does_not_change_forbidden_files(self):
        ...
        allowed = {
            ...
            (
                ...
                "witness_core_evidence_external_receipt_manifest_decision_audit_work_order_readiness.py"
            ),
            (
                "backend/apps/calculations/management/commands/"
                "build_witness_core_evidence_external_receipt_manifest_decision_audit_work_order_readiness_report.py"
            ),
            (
                "backend/apps/calculations/"
                "test_witness_core_evidence_external_receipt_manifest_decision_audit_work_order_readiness.py"
            ),
        }
        for path in changed:
    ...
    def setUp(self):
        self.tmp_dir = Path(__file__).resolve().parents[3] / ".tmp" / "test-p87"
...
def _expected_summary():
    return {
        "source_case_decision_audit_work_order_rows": 5,
        "source_attachment_decision_audit_work_order_rows": 10,
...
def _p85_payload():
    ...
    for case_index, case_id in enumerate(CASE_IDS, start=1):
        ...
        for family in FAMILY_ORDER:
            ...
            attachment_rows.append(
                {
                    "attachment_decision_audit_work_order_index": attachment_index,
                    "case_decision_audit_work_order_index": case_index,
                    "source_attachment_receipt_manifest_decision_audit_index": attachment_index,
                    "case_id": case_id,
        ...
        case_rows.append(
            {
                "case_decision_audit_work_order_index": case_index,
                "case_id": case_id,
                "source_case_receipt_manifest_decision_audit_index": case_index,
                "source_attachment_receipt_manifest_decision_audit_indices": attachment_indices,
                "attachment_decision_audit_work_order_indices": attachment_indices,
                "decision_audit_work_order_status": "blocked_pending_external_evidence_receipt_manifest_decision_audit_work_orders",
                "decision_audit_status": "blocked_pending_external_evidence_receipt_manifest_decision_audit",
                "decision_queue_status": "blocked_pending_external_evidence_receipt_manifest_decision",
                "external_receipt_gate_status": "blocked_pending_external_evidence_receipts",
    ...
    return {
        ...
        "case_decision_audit_work_order_rows": case_rows,
        "attachment_decision_audit_work_order_rows": attachment_rows,
    }
