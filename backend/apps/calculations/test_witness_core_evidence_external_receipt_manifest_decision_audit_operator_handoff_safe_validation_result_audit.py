...
from apps.calculations.witness_core_evidence_external_receipt_manifest_decision_audit_operator_handoff_safe_validation_result_audit import (
    build_witness_core_evidence_external_receipt_manifest_decision_audit_operator_handoff_safe_validation_result_audit_report,
)
...
] + RESULT_FAMILY_LABELS + AUDIT_FAMILY_LABELS


class ExternalReceiptManifestDecisionAuditOperatorHandoffSafeValidationResultAuditTests(SimpleTestCase):
    def test_result_audit_preserves_p93_order_and_blocks_audits(self):
        ledger_path = self._write_payload("safe-validation-result-ledger.json", _p93_payload())

        report = build_witness_core_evidence_external_receipt_manifest_decision_audit_operator_handoff_safe_validation_result_audit_report(
            external_receipt_manifest_decision_audit_operator_handoff_safe_validation_result_ledger_report_path=ledger_path,
        )
        ...
        self.assertEqual(report["stage"], "P95-A")
        self.assertEqual(
            report["status"],
        ...
        self.assertEqual([row["case_id"] for row in report["safe_validation_result_audit_case_rows"]], CASE_IDS)
        self.assertEqual([row["case_id"] for row in report["safe_validation_result_audit_attachment_rows"][::2]], CASE_IDS)
        self.assertEqual(
            [row["evidence_family"] for row in report["safe_validation_result_audit_attachment_rows"][:4]],
            FAMILY_ORDER + FAMILY_ORDER,
        )
        self.assertEqual(len(report["safe_validation_result_audit_rows"]), 40)
        self.assertEqual(
            [row["safe_validation_command_family"] for row in report["safe_validation_result_audit_rows"][:4]],
            SAFE_VALIDATION_FAMILIES,
        )
        self.assertEqual(
            [row["safe_validation_result_family_label"] for row in report["safe_validation_result_audit_rows"][:4]],
            RESULT_FAMILY_LABELS,
        )
        self.assertEqual(
            [row["safe_validation_result_audit_family_label"] for row in report["safe_validation_result_audit_rows"][:4]],
            AUDIT_FAMILY_LABELS,
        )
        self.assertTrue(
            all(
                row["safe_validation_result_audit_execution_status"] == "not_executed"
                for row in report["safe_validation_result_audit_rows"]
            )
        )
        self.assertTrue(
            all(
                row["safe_validation_result_audit_record_status"] == "not_recorded"
                for row in report["safe_validation_result_audit_rows"]
            )
        )
        self.assertTrue(all(row["ready_to_attach"] is False for row in report["safe_validation_result_audit_rows"]))
        self.assertTrue(all(row["ready_to_mark"] is False for row in report["safe_validation_result_audit_rows"]))
        self.assertTrue(_contains_exact_marker_labels(report))
        self.assertTrue(_serialized_safe(report))

    def test_command_writes_json_without_mutating_p93_or_running_external_actions(self):
        ledger_path = self._write_payload("safe-validation-result-ledger.json", _p93_payload())
        output_path = self.tmp_dir / "safe-validation-result-audit.json"
        before = ledger_path.read_text(encoding="utf-8")

        with (
            mock.patch("subprocess.Popen", side_effect=AssertionError("P95 must not run external tools")),
            mock.patch("subprocess.run", side_effect=AssertionError("P95 must not run commands")),
            mock.patch("os.system", side_effect=AssertionError("P95 must not run shell commands")),
        ):
            call_command(
                "build_witness_core_evidence_external_receipt_manifest_decision_audit_operator_handoff_safe_validation_result_audit_report",
                "--external-receipt-manifest-decision-audit-operator-handoff-safe-validation-result-ledger-report",
                str(ledger_path),
                "--output",
                str(output_path),
            )
        ...
        self.assertEqual(written["stage"], "P95-A")
    ...
    def test_committed_p95_artifact_is_safe_and_current(self):
        ...
        ledger = _read_json(
            ...
            / (
                "core-evidence-external-receipt-manifest-decision-audit-operator-handoff-"
                "safe-validation-result-ledger-p93-report.json"
            )
        )
        ...
        self.assertEqual(report["summary"], _expected_summary())
        self.assertEqual([row["case_id"] for row in report["safe_validation_result_audit_case_rows"]], CASE_IDS)
        self.assertEqual([row["case_id"] for row in ledger["safe_validation_result_ledger_case_rows"]], CASE_IDS)
        self.assertEqual([row["case_id"] for row in report["safe_validation_result_audit_attachment_rows"][::2]], CASE_IDS)
        self.assertEqual(len(report["safe_validation_result_audit_rows"]), 40)
        self.assertTrue(_contains_exact_marker_labels(report))
        self.assertTrue(_serialized_safe(report))
    ...
    def test_stage_does_not_change_forbidden_files(self):
        ...
        allowed = {
            ...
            (
                ...
                "witness_core_evidence_external_receipt_manifest_decision_audit_operator_handoff_safe_validation_result_audit.py"
            ),
            (
                "backend/apps/calculations/management/commands/"
                "build_witness_core_evidence_external_receipt_manifest_decision_audit_operator_handoff_safe_validation_result_audit_report.py"
            ),
            (
                "backend/apps/calculations/"
                "test_witness_core_evidence_external_receipt_manifest_decision_audit_operator_handoff_safe_validation_result_audit.py"
            ),
        }
        for path in changed:
    ...
    def setUp(self):
        self.tmp_dir = Path(__file__).resolve().parents[3] / ".tmp" / "test-p95"
...
def _expected_summary():
    return {
        "source_safe_validation_result_ledger_case_rows": 5,
        "source_safe_validation_result_ledger_attachment_rows": 10,
        "source_safe_validation_result_ledger_rows": 40,
