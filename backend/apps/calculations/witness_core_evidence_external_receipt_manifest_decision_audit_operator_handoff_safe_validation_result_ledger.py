...
BLOCKED_NOTE = (
    "Safe validation result ledger rows are labels only; no command execution, result recording, "
    "operator handoff, work-order delivery, notification, ticket, upload, attachment, mark, accept, "
    "reject, or defer action is executed."
)
RESULT_BLOCKED_NOTE = (
    "Safe validation result ledger row is blocked pending external evidence and operator handoff; "
    "the validation family is a label only and no result is recorded."
)


def build_witness_core_evidence_external_receipt_manifest_decision_audit_operator_handoff_safe_validation_result_ledger_report(
    *,
    external_receipt_manifest_decision_audit_operator_handoff_safe_validation_transcript_report_path: str | Path,
) -> dict[str, Any]:
    source = _read_json(Path(external_receipt_manifest_decision_audit_operator_handoff_safe_validation_transcript_report_path))
    source_summary = source.get("summary") if isinstance(source.get("summary"), dict) else {}
    case_rows = [
        row for row in source.get("safe_validation_transcript_case_rows", []) if isinstance(row, dict)
    ]
    attachment_rows = [
        row for row in source.get("safe_validation_transcript_attachment_rows", []) if isinstance(row, dict)
    ]
    transcript_command_rows = [
        row for row in source.get("safe_validation_transcript_command_rows", []) if isinstance(row, dict)
    ]

    ledger_rows = [
        _ledger_row(row, safe_validation_result_ledger_index=index)
        for index, row in enumerate(transcript_command_rows, start=1)
    ]
    ledger_indices_by_attachment = _ledger_indices_by_attachment(ledger_rows)
    ledger_attachment_rows = [
        _attachment_row(
            row,
            safe_validation_result_ledger_attachment_index=index,
            ledger_indices=ledger_indices_by_attachment.get(index, []),
        )
        for index, row in enumerate(attachment_rows, start=1)
    ]
    attachment_indices_by_case = _attachment_indices_by_case(ledger_attachment_rows)
    ledger_indices_by_case = _ledger_indices_by_case(ledger_rows)
    ledger_case_rows = [
        _case_row(
            row,
            safe_validation_result_ledger_case_index=index,
            attachment_indices=attachment_indices_by_case.get(index, []),
            ledger_indices=ledger_indices_by_case.get(index, []),
        )
        for index, row in enumerate(case_rows, start=1)
    ]
    summary = _summary(source_summary, ledger_case_rows, ledger_attachment_rows, ledger_rows)

    return {
        "schema_version": SCHEMA_VERSION,
        "domain": DOMAIN_KEY,
        "stage": STAGE,
        "status": STATUS if ledger_case_rows else "empty",
        "upstream_safe_validation_transcript_schema_version": _safe_source_marker(
            source.get("schema_version"),
            UPSTREAM_TRANSCRIPT_SCHEMA_VERSION,
        ),
        "upstream_safe_validation_transcript_stage": _safe_source_marker(source.get("stage"), UPSTREAM_TRANSCRIPT_STAGE),
        ...
        "status_labels": RESULT_LEDGER_STATUS_LABELS,
        "safety_labels": SAFETY_LABELS,
        "count_labels": _count_labels(summary),
        "operator_note": BLOCKED_NOTE,
        "summary": summary,
        "safe_validation_result_ledger_case_rows": ledger_case_rows,
        "safe_validation_result_ledger_attachment_rows": ledger_attachment_rows,
        "safe_validation_result_ledger_rows": ledger_rows,
    }
...
def _summary(
    ...
    return {
        "source_safe_validation_transcript_case_rows": len(case_rows),
        "source_safe_validation_transcript_attachment_rows": len(attachment_rows),
        "source_safe_validation_transcript_command_rows": len(ledger_rows),
        "safe_validation_transcript_case_rows": _safe_int(
            source_summary.get("safe_validation_transcript_case_rows")
        ),
        ...
        "safe_validation_result_ledger_rows": len(ledger_rows),
...
def _case_row(
    row: dict[str, Any],
    *,
    safe_validation_result_ledger_case_index: int,
    attachment_indices: list[int],
    ledger_indices: list[int],
) -> dict[str, Any]:
    return {
        "safe_validation_result_ledger_case_index": safe_validation_result_ledger_case_index,
        "case_id": str(row.get("case_id") or ""),
        "source_safe_validation_transcript_case_index": _safe_int(
            row.get("safe_validation_transcript_case_index")
        )
        or safe_validation_result_ledger_case_index,
        "source_safe_validation_transcript_attachment_indices": [
            _safe_int(value) for value in row.get("safe_validation_transcript_attachment_indices", [])
        ],
        "safe_validation_result_ledger_attachment_indices": attachment_indices,
        "safe_validation_result_ledger_indices": ledger_indices,
        "safe_validation_result_ledger_status": STATUS,
        ...
        "safety_labels": SAFETY_LABELS,
        "safe_next_action_labels": SAFE_NEXT_ACTION_LABELS,
        "blocked_reason_label": "blocked_no_external_evidence_operator_handoff_or_safe_validation_result",
...
def _attachment_row(
    row: dict[str, Any],
    *,
    safe_validation_result_ledger_attachment_index: int,
    ledger_indices: list[int],
) -> dict[str, Any]:
    family = _safe_evidence_family(row.get("evidence_family"))
    return {
        "safe_validation_result_ledger_attachment_index": safe_validation_result_ledger_attachment_index,
        "safe_validation_result_ledger_case_index": _safe_int(row.get("safe_validation_transcript_case_index")),
        "source_safe_validation_transcript_attachment_index": _safe_int(
            row.get("safe_validation_transcript_attachment_index")
        ),
        "source_safe_validation_transcript_case_index": _safe_int(row.get("safe_validation_transcript_case_index")),
        "safe_validation_result_ledger_indices": ledger_indices,
        "case_id": str(row.get("case_id") or ""),
...
def _ledger_row(row: dict[str, Any], *, safe_validation_result_ledger_index: int) -> dict[str, Any]:
    family = _safe_evidence_family(row.get("evidence_family"))
    ...
    return {
        "safe_validation_result_ledger_index": safe_validation_result_ledger_index,
        "source_safe_validation_transcript_command_index": _safe_int(
            row.get("safe_validation_transcript_command_index")
        )
        or safe_validation_result_ledger_index,
        "safe_validation_result_ledger_attachment_index": attachment_index,
        "safe_validation_result_ledger_case_index": case_index,
        "source_safe_validation_transcript_attachment_index": attachment_index,
        "source_safe_validation_transcript_case_index": case_index,
        "case_id": str(row.get("case_id") or ""),
        "evidence_family": family,
        "external_tool_label": _external_tool_label(family),
        "operator_label": str(row.get("operator_label") or _operator_label(family)),
        ...
        "safe_validation_result_family_label": f"{command_family}_blocked_result_label_only",
        "safe_validation_result_ledger_status": STATUS,
        "safe_validation_transcript_status": _safe_transcript_status(row.get("safe_validation_transcript_status")),
        "safe_validation_command_status": SAFE_VALIDATION_COMMAND_STATUS,
        "safe_validation_command_execution_status": SAFE_VALIDATION_COMMAND_EXECUTION_STATUS,
        ...
        **_safety_flags(),
        ...
        "safe_next_action_labels": SAFE_NEXT_ACTION_LABELS,
        "blocked_reason_label": "blocked_no_external_evidence_operator_handoff_or_safe_validation_result",
        "blocked_note": RESULT_BLOCKED_NOTE,
    }


def _ledger_status_fields() -> dict[str, Any]:
    return {
...
def _ledger_indices_by_attachment(rows: list[dict[str, Any]]) -> dict[int, list[int]]:
    grouped: dict[int, list[int]] = defaultdict(list)
    for row in rows:
        attachment_index = _safe_int(row.get("safe_validation_result_ledger_attachment_index"))
        ledger_index = _safe_int(row.get("safe_validation_result_ledger_index"))
        if attachment_index and ledger_index:
            grouped[attachment_index].append(ledger_index)
    return dict(grouped)
...
def _ledger_indices_by_case(rows: list[dict[str, Any]]) -> dict[int, list[int]]:
    grouped: dict[int, list[int]] = defaultdict(list)
    for row in rows:
        case_index = _safe_int(row.get("safe_validation_result_ledger_case_index"))
        ledger_index = _safe_int(row.get("safe_validation_result_ledger_index"))
        if case_index and ledger_index:
            grouped[case_index].append(ledger_index)
    return dict(grouped)
...
def _operator_label(family: str) -> str:
    return (
        "await_jhora_safe_validation_result_ledger"
        if family == "jhora_screenshot_or_packet"
        else "await_parashara_light_safe_validation_result_ledger"
    )
