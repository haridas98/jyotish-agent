...
BLOCKED_NOTE = (
    "Receipt manifest acceptance is blocked; no receipt manifest is received or accepted; "
    "rows are labels only and no upload, attachment, or mark command is executed."
)
SLOT_BLOCKED_NOTE = (
    "Receipt manifest is not received; acceptance and rejection are not started; "
    "no evidence file, evidence hash, upload, attachment, or mark command was executed."
)


def build_witness_core_evidence_external_receipt_manifest_acceptance_gate_report(
    *,
    external_receipt_manifest_preflight_report_path: str | Path,
) -> dict[str, Any]:
    source = _read_json(Path(external_receipt_manifest_preflight_report_path))
    source_summary = source.get("summary") if isinstance(source.get("summary"), dict) else {}
    case_preflight_rows = [
        row for row in source.get("case_receipt_manifest_preflight_rows", []) if isinstance(row, dict)
    ]
    slot_preflight_rows = [
        row for row in source.get("attachment_receipt_manifest_preflight_rows", []) if isinstance(row, dict)
    ]
    slots_by_case = _slots_by_case(slot_preflight_rows)

    case_rows = [
        _case_acceptance_row(
            row,
            case_receipt_manifest_acceptance_gate_index=index,
            source_slots=slots_by_case.get(_safe_int(row.get("case_receipt_manifest_preflight_index")) or index, []),
        )
        for index, row in enumerate(case_preflight_rows, start=1)
    ]
    slot_rows = [
        _attachment_acceptance_row(row, attachment_receipt_manifest_acceptance_gate_index=index)
        for index, row in enumerate(slot_preflight_rows, start=1)
    ]
    ...
    return {
        ...
        "upstream_preflight_stage": _safe_source_marker(source.get("stage"), UPSTREAM_PREFLIGHT_STAGE),
        "upstream_preflight_status": _safe_source_marker(source.get("status"), UPSTREAM_PREFLIGHT_STATUS),
        ...
        "acceptance_criteria_labels": ACCEPTANCE_CRITERIA_LABELS,
        ...
        "summary": {
            "source_case_receipt_manifest_preflight_rows": _safe_int(
                source_summary.get("case_receipt_manifest_preflight_rows", len(case_preflight_rows))
            ),
            "source_attachment_receipt_manifest_preflight_rows": _safe_int(
                source_summary.get("attachment_receipt_manifest_preflight_rows", len(slot_preflight_rows))
            ),
            "case_receipt_manifest_acceptance_gate_rows": len(case_rows),
            "attachment_receipt_manifest_acceptance_gate_rows": len(slot_rows),
            "pending_external_evidence_receipt_manifest_acceptance_count": len(slot_rows),
            "pending_jhora_receipt_manifest_acceptance_count": sum(
                1 for row in slot_rows if row["evidence_family"] == "jhora_screenshot_or_packet"
            ),
            "pending_parashara_light_receipt_manifest_acceptance_count": sum(
                1
...
def _case_acceptance_row(
    row: dict[str, Any],
    *,
    case_receipt_manifest_acceptance_gate_index: int,
    source_slots: list[dict[str, Any]],
) -> dict[str, Any]:
    source_slot_indices = [
        _safe_int(slot.get("attachment_receipt_manifest_preflight_index")) for slot in source_slots
    ]
    families = {str(slot.get("evidence_family") or "") for slot in source_slots}
    return {
        "case_receipt_manifest_acceptance_gate_index": case_receipt_manifest_acceptance_gate_index,
        "case_id": str(row.get("case_id") or ""),
        "source_case_receipt_manifest_preflight_index": _safe_int(
            row.get("case_receipt_manifest_preflight_index")
        )
        or case_receipt_manifest_acceptance_gate_index,
        "source_attachment_receipt_manifest_preflight_indices": source_slot_indices,
        ...
        "pending_external_evidence_receipt_manifests": len(REQUIRED_EVIDENCE_FAMILIES),
...
def _attachment_acceptance_row(
    row: dict[str, Any],
    *,
    attachment_receipt_manifest_acceptance_gate_index: int,
) -> dict[str, Any]:
    family = _safe_evidence_family(row.get("evidence_family"))
    return {
        "attachment_receipt_manifest_acceptance_gate_index": attachment_receipt_manifest_acceptance_gate_index,
        "case_receipt_manifest_acceptance_gate_index": _safe_int(row.get("case_receipt_manifest_preflight_index")),
        "source_attachment_receipt_manifest_preflight_index": _safe_int(
            row.get("attachment_receipt_manifest_preflight_index")
        ),
        "case_id": str(row.get("case_id") or ""),
...
def _slots_by_case(rows: list[dict[str, Any]]) -> dict[int, list[dict[str, Any]]]:
    ...
    for row in rows:
        index = _safe_int(row.get("case_receipt_manifest_preflight_index"))
        if index:
            grouped[index].append(row)
    for case_rows in grouped.values():
        case_rows.sort(key=lambda row: _safe_int(row.get("attachment_receipt_manifest_preflight_index")))
    return dict(grouped)
