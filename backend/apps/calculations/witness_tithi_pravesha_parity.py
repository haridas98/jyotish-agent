...
PAYLOAD_KEYS = ("tithi_pravesha", "tithi_pravesha_report", "annual_return", "varshaphala", "tajaka")
...
def build_witness_tithi_pravesha_parity_report(
    *,
    ...
    return {
        ...
        "metadata": {
            ...
            "diagnostic_policy": (
                "Reviewed witness Tithi Pravesha rows are compared with Jyotish Agent annual return payloads."
            ),
            "normalization_notes": [
                "Tithi Pravesha, annual_return, varshaphala and tajaka witness payloads are normalized.",
                "Solar-lunar angle and degree fields use 0.01 degree tolerance.",
                "Unknown optional annual return fields are skipped unless no comparable fields remain.",
            ],
...
def render_tithi_pravesha_parity_markdown(report: dict[str, Any]) -> str:
    summary = report["summary"]
    lines = [
        "# Tithi Pravesha Parity Report",
        "",
        "Diagnostic comparison of reviewed witness annual return rows against Jyotish Agent Tithi Pravesha payloads.",
    ...
    if not field_results:
        if witness_missing_records == len(reviewed_records):
            ...
            row = _empty_case(
                ...
                sources_present,
                review_statuses,
                sorted(set(missing_fields or ["tithi_pravesha.comparable_fields"])),
            )
...
def _empty_case(
    ...
    return {
        ...
        "checked_layers": [],
        "failed_layers": [],
        "missing_layers": ["tithi_pravesha"] if missing_fields else [],
        "checked_fields": [],
        "failed_fields": [],
...
def _compare_record(record: dict[str, Any]) -> tuple[list[dict[str, Any]], list[str], list[str], str]:
    ...
    if not expected and not skipped_fields:
        return [], [f"{record['source']}.tithi_pravesha"], [], "witness"
    if not actual:
        return [], ["calculated.tithi_pravesha"], skipped_fields, "calculated"
    results, missing = _compare_return(expected, actual, str(record["source"]))
    if not results and skipped_fields:
        return [], ["tithi_pravesha.comparable_fields"], skipped_fields, "unsupported"
    if not results:
        return [], missing or ["calculated.tithi_pravesha"], skipped_fields, "calculated"
...
def _actual_return(chart: dict[str, Any], fixture: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    for candidate in fallback_inputs:
        if not _has_return_input(candidate):
            continue
        try:
            from .workflows import build_tithi_pravesha_report

            payload, _skipped = _coerce_return(build_tithi_pravesha_report(candidate))
            if payload:
                return payload
        except Exception:
            continue
...
def _return_from_container(container: dict[str, Any]) -> dict[str, Any]:
    if not isinstance(container, dict):
        return {}
    for key in ("tithi_pravesha", "tithi_pravesha_report", "annual_return", "varshaphala"):
    ...
    payload, _skipped = _coerce_return(tajaka_report.get("tithi_pravesha"))
    return payload
...
def _coerce_return(value: Any) -> tuple[dict[str, Any], list[str]]:
    ...
    context = value.get("return_context") if isinstance(value.get("return_context"), dict) else {}
    if context:
        output["return_context"] = {str(key): _normalized_text(val) for key, val in context.items() if _normalized_text(val)}
    output["natal"] = {"solar_lunar_angle": _angle_from_value(value.get("natal"), "solar_lunar_angle")}
    solar = value.get("solar_lunar_angle") if isinstance(value.get("solar_lunar_angle"), dict) else {}
    output["return"] = _coerce_return_moment(value.get("return"), value.get("return_moment"), solar)
    annual_context = value.get("annual_context") if isinstance(value.get("annual_context"), dict) else {}
    return_chart = value.get("return_chart") if isinstance(value.get("return_chart"), dict) else {}
    output["annual_context"] = {
        "lagna": _coerce_placement(annual_context.get("lagna") or return_chart.get("lagna") or return_chart.get("ascendant")),
        "sun": _coerce_placement(annual_context.get("sun") or return_chart.get("sun")),
        "moon": _coerce_placement(annual_context.get("moon") or return_chart.get("moon")),
        "panchanga": _coerce_panchanga(annual_context.get("panchanga") or value.get("annual_panchanga") or {}),
        "tajaka": _coerce_tajaka(annual_context.get("tajaka") or value.get("tajaka") or {}),
    }
    skipped = [f"tithi_pravesha.{_normalized_key(key)}" for key in value if _normalized_key(key) not in KNOWN_RETURN_KEYS]
...
def _compare_return(expected: dict[str, Any], actual: dict[str, Any], source: str) -> tuple[list[dict[str, Any]], list[str]]:
    ...
    missing: list[str] = []
    _compare_scalar(results, source, "target_year", "target_year", expected.get("target_year"), actual.get("target_year"))
    for key, value in (expected.get("return_context") or {}).items():
        _compare_scalar(results, source, "return_context", f"return_context.{_normalized_key(key)}", value, (actual.get("return_context") or {}).get(key))
    _compare_scalar(
        results,
        source,
        "solar_lunar_angle",
        "natal.solar_lunar_angle",
        (expected.get("natal") or {}).get("solar_lunar_angle"),
        (actual.get("natal") or {}).get("solar_lunar_angle"),
    )
    expected_return = expected.get("return") if isinstance(expected.get("return"), dict) else {}
    actual_return = actual.get("return") if isinstance(actual.get("return"), dict) else {}
    for key in ("solar_lunar_angle", "delta_degrees"):
        _compare_scalar(results, source, "solar_lunar_angle", f"return.{key}", expected_return.get(key), actual_return.get(key))
    for key in ("date", "time", "local_datetime", "timezone"):
        _compare_scalar(results, source, "return_moment", f"return.{key}", expected_return.get(key), actual_return.get(key))
    expected_context = expected.get("annual_context") if isinstance(expected.get("annual_context"), dict) else {}
    actual_context = actual.get("annual_context") if isinstance(actual.get("annual_context"), dict) else {}
    for body in ("lagna", "sun", "moon"):
        for key in ("rashi", "degree"):
            _compare_scalar(
                results,
                source,
                "return_chart",
                f"annual_context.{body}.{key}",
                (expected_context.get(body) or {}).get(key),
                (actual_context.get(body) or {}).get(key),
            )
    for key in PANCHANGA_FIELDS:
        _compare_scalar(
            results,
            ...
            (expected_context.get("panchanga") or {}).get(key),
            (actual_context.get("panchanga") or {}).get(key),
        )
    expected_tajaka = expected_context.get("tajaka") if isinstance(expected_context.get("tajaka"), dict) else {}
    actual_tajaka = actual_context.get("tajaka") if isinstance(actual_context.get("tajaka"), dict) else {}
    _compare_scalar(results, source, "tajaka_baseline", "tajaka.muntha.rashi", (expected_tajaka.get("muntha") or {}).get("rashi"), (actual_tajaka.get("muntha") or {}).get("rashi"))
    _compare_scalar(
        results,
        source,
        "tajaka_baseline",
        "tajaka.muntha.house_from_annual_lagna",
        (expected_tajaka.get("muntha") or {}).get("house_from_annual_lagna"),
        (actual_tajaka.get("muntha") or {}).get("house_from_annual_lagna"),
    )
    _compare_scalar(results, source, "tajaka_baseline", "tajaka.status", expected_tajaka.get("status"), actual_tajaka.get("status"))
...
def _layer_from_field(field: str) -> str:
    if field == "target_year":
        return "target_year"
    if field.startswith("return_context."):
        return "return_context"
    if field.startswith("natal.") or field in {"return.solar_lunar_angle", "return.delta_degrees"}:
        return "solar_lunar_angle"
    if field.startswith("return."):
        return "return_moment"
    if field.startswith("annual_context."):
        return "return_chart"
    if field.startswith("annual_panchanga."):
        return "annual_panchanga"
    if field.startswith("tajaka."):
        return "tajaka_baseline"
    return ""
