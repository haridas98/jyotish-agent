...
New capture queues also include `next_item`, `next_step_label` and `next_command` so the current CLI capture step can be
shown without opening the markdown file. Review/status actions are exposed as a `manual_review_command` preflight command
on the item; only the preflight/seal output should be used for commands that actually mark evidence reviewed.
