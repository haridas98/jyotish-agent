...
| Parashara Light witness inventory | manual-template-ready | Phase 2 | PL7 UI metadata/screenshot capture and verification-packet command exist for the running `PL7.exe` session. Packet reports now accept `manual_witness_values`, compare copied/manual PL values against our chart fields, and can generate an editable Lagna/graha witness template from a packet. Qt panes expose mostly generic `QWidget`, so PL artifacts remain draft metadata/screenshot/manual-export witnesses until copyable tables are identified. |
